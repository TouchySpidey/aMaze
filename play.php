...
if (isset($_SESSION['username']) && isset($_SESSION['labirinto']) && isset($_SESSION['avversario'])) {
	if (isset($_SESSION['mosse'])) {
		...
		if (isset($_POST['cella_cliccata'])) {
			...
			if ($i == $old_i) {
				...
				if ($j < $old_j) {
					...
					while ($old_j > $j) {
						//echo intval($_SESSION['maze'] [$old_i][$old_j]['O']);
						$passa = !$_SESSION['maze'] [$old_i][$old_j]['O'] && $passa;
						$old_j--;
					}
				}
				if ($j > $old_j) {
					//stiamo andando a sinistra
					while ($old_j < $j) {
						$passa = !$_SESSION['maze'] [$old_i][$old_j]['E'] && $passa;
						$old_j++;
					}
				}
			} elseif ($j == $old_j) {
				...
				if ($i < $old_i) {
					...
					while ($old_i > $i) {
						$passa = !$_SESSION['maze'] [$old_i][$old_j]['N'] && $passa;
						$old_i--;
					}
				}
				if ($i > $old_i) {
					...
					while ($old_i < $i) {
						$passa = !$_SESSION['maze'] [$old_i][$old_j]['S'] && $passa;
						$old_i++;
					}
				}
			}
			if ($passa) {
				...
				$_SESSION['mosse']++;
				$h = count($_SESSION['maze'] );
				$w = count($_SESSION['maze'] [0]);
				if ($i == $h-1 && $j == $w-1) {
		...
		fclose($pointer);
		$_SESSION['maze'] = json_decode($tuttoilfile, true)[$_SESSION['labirinto']];
	}
?>
<html>
	<head>
